...
export function Home({ locale }: { locale: Locale }) {
  ...
  return (
    <>
      ...
      <section className="masters-preview section-space reveal" aria-labelledby="masters-title"><div className="section-heading"><p className="eyebrow">{isRu ? "Команда" : "Команда"}</p><h2 id="masters-title">{isRu ? "Роли команды" : "Команда рөлдері"}</h2><Link href={`/${locale}/masters`}>{isRu ? "Смотреть роли" : "Рөлдерді көру"} <ArrowUpRight aria-hidden="true" /></Link></div><p className="lede">{isRu ? "Состав мастеров пока представлен по профессиональным ролям." : "Шеберлер құрамы әзірге кәсіби рөлдер бойынша көрсетілген."}</p><div className="role-marquee" aria-label={isRu ? "Роли специалистов" : "Маман рөлдері"}><div>{[...specialists, ...specialists].map((specialist, index) => <span key={`${specialist.id}-${index}`}>{specialist.role[locale]} <i>·</i></span>)}</div></div></section>
